<template>
  <div class="modify-pass">
    ...
    <div style="margin-top:44px">
      <div class="white-bg">
        <x-input title="旧登录密码" :is-type="passValidator" required type="text" placeholder="输入旧密码" v-model="originPass" :min="6" :max="16" @on-change="handleInputChange"></x-input>
      </div>
      <div class=" intro">
        为了您的账号安全,请输入<span>6-16位字母数字组合密码</span>
      </div>
      <div class="white-bg">
        <x-input title="新登录密码" :is-type="passValidator" required :icon-type="iconType" type="text" placeholder="请输入6-16位字母数字组合密码" v-model="newPass1" :min="6" :max="16" @on-change="handleInputChange"></x-input>
        <x-input title="确认新密码" :is-type="passValidator" required v-model="newPass2" type="text" placeholder="请确认新密码" :equal-with="newPass1" :min="6" :max="16" @on-change="handleInputChange"></x-input>
      </div>
      <div>
